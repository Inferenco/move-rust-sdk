...
pub struct AptosConfig {
    ...
    /// Optional override for the chain ID. When `Some`, [`Self::chain_id`]
    /// returns this value regardless of the [`Network`] variant. This is
    /// how the `move-rust-sdk` crate injects a non-Aptos chain ID
    /// (e.g. Movement testnet's `250`) into an otherwise Aptos-shaped
    /// configuration. When `None`, the chain ID is derived from
    /// [`Network::chain_id`].
    pub(crate) chain_id_override: Option<ChainId>,
}
...
impl AptosConfig {
    ...
    /// Construct an `AptosConfig` from already-parsed fields. This is
    /// the public, structural constructor used by the
    /// [`move-rust-sdk`](https://docs.rs/move-rust-sdk) crate to
    /// bridge its chain-agnostic `MoveConfig` into an
    /// `AptosConfig` carrying a non-Aptos fullnode URL (e.g. a
    /// Movement or custom Move-chain node).
    ///
    /// # Security
    ///
    /// All URLs are validated with [`validate_url_scheme`] — only
    ...
    pub fn from_parts(
        ...
        validate_url_scheme(&fullnode_url)?;
        if let Some(ref u) = indexer_url {
            validate_url_scheme(u)?;
        }
        if let Some(ref u) = faucet_url {
            validate_url_scheme(u)?;
        }
        Ok(Self {
            network: Network::Custom,
    ...
    ///
    /// If a chain-ID override has been set via [`Self::set_chain_id`],
    /// that value is returned; otherwise the well-known chain ID for
    /// the active [`Network`] variant is returned.
    pub fn chain_id(&self) -> ChainId {
        self.chain_id_override
            .unwrap_or_else(|| self.network.chain_id())
    }

    /// Override the chain ID. This is intended for forks and other
    /// Move-based chains that re-use the Aptos wire format but with a
    /// different chain ID (e.g. Movement's `126` for mainnet, `250`
    /// for testnet). The [`move-rust-sdk`](crate) crate uses this to
    /// inject a non-Aptos chain ID into an `AptosConfig` without
    /// having to fork the configuration type.
    ///
    /// Pass `ChainId::new(0)` to clear the override and fall back to
    /// the value derived from the [`Network`] variant.
    ///
    /// # Example
    ///
    /// ```rust
    /// use move_core_sdk::{AptosConfig, types::ChainId};
    ///
    /// // Use Movement testnet (chain ID 250) on top of the
    /// // Aptos wire format.
    /// let cfg = AptosConfig::custom("https://testnet.movementnetwork.xyz/v1")
    ///     .unwrap()
    ///     .set_chain_id(ChainId::new(250));
    /// assert_eq!(cfg.chain_id().id(), 250);
    /// ```
    #[must_use]
    pub fn set_chain_id(mut self, chain_id: ChainId) -> Self {
        self.chain_id_override = if chain_id.id() == 0 {
            None
        } else {
            Some(chain_id)
        };
