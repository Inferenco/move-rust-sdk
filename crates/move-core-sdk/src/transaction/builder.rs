...
/// use move_core_sdk::types::{AccountAddress, ChainId};
///
...
impl TransactionBuilder {
    ...
    /// Like [`build_and_sign`](Self::build_and_sign) but with a
    /// caller-supplied signing domain.
    ///
    /// The default signing domain is `"APTOS::RawTransaction"`. For
    /// Movement (and any other Move-based chain) the caller passes
    /// its own domain (e.g. `"MOVEMENT::RawTransaction"`).
    ///
    /// # Errors
    ///
    /// Returns an error if the transaction cannot be built or signed.
    #[cfg(feature = "ed25519")]
    pub fn build_and_sign_with_domain<A: Account>(
        self,
        account: &A,
        domain: &[u8],
...
pub fn sign_transaction<A: Account>(
    ...
    sign_transaction_with_domain(raw_txn, account, b"APTOS::RawTransaction")
}

/// Like [`sign_transaction`] but with a caller-supplied signing
/// domain. Use this for non-Aptos Move-based chains whose signing
/// domain differs from `"APTOS::RawTransaction"`.
///
/// # Errors
///
/// Returns an error if generating the signing message fails or if
/// the account fails to sign.
pub fn sign_transaction_with_domain<A: Account>(
    raw_txn: &RawTransaction,
    account: &A,
    domain: &[u8],
) -> AptosResult<SignedTransaction> {
    let signing_message = raw_txn.signing_message_with_domain(domain)?;
...
/// Like [`sign_multi_agent_transaction`] but with a caller-supplied
...
pub fn sign_multi_agent_transaction_with_domain<A: Account>(
    multi_agent: &MultiAgentRawTransaction,
    sender: &A,
    secondary_signers: &[&dyn Account],
    domain: &[u8],
) -> AptosResult<SignedTransaction> {
    let signing_message = multi_agent.signing_message_with_domain(domain)?;
...
/// Like [`sign_fee_payer_transaction`] but with a caller-supplied
/// signing domain. Use this for non-Aptos Move-based chains whose
/// signing domain differs from `"APTOS::RawTransactionWithData"`.
///
/// # Errors
...
/// Returns an error if generating the signing message fails or if
/// any signer fails to sign.
pub fn sign_fee_payer_transaction_with_domain<A: Account>(
    fee_payer_txn: &FeePayerRawTransaction,
    sender: &A,
    secondary_signers: &[&dyn Account],
    fee_payer: &dyn Account,
