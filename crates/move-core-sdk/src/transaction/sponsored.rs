...
///
/// ```rust,ignore
/// use move_core_sdk::transaction::sign_sponsored_transaction;
///
    ...
    F: Account,
{
    sign_sponsored_transaction_with_domain(
        ...
        secondary_signers,
        fee_payer,
        b"APTOS::RawTransactionWithData",
    )
}

/// Like [`sign_sponsored_transaction`] but with a caller-supplied
/// signing domain. Use this for non-Aptos Move-based chains whose
/// signing domain differs from `"APTOS::RawTransactionWithData"`.
///
/// # Errors
///
/// Returns an error if generating the signing message fails or if
/// any signer fails to sign.
pub fn sign_sponsored_transaction_with_domain<S, F>(
    fee_payer_txn: &FeePayerRawTransaction,
    sender: &S,
    secondary_signers: &[&dyn Account],
    fee_payer: &F,
...
where
    ...
    F: Account,
{
    let signing_message = fee_payer_txn.signing_message_with_domain(domain)?;
...
impl PartiallySigned {
    ...
    pub fn sign_as_sender<A: Account>(&mut self, sender: &A) -> AptosResult<()> {
        self.sign_as_sender_with_domain(sender, b"APTOS::RawTransactionWithData")
    }

    /// Like [`sign_as_sender`](Self::sign_as_sender) but with a
    /// caller-supplied signing domain.
    ///
    /// # Errors
    ///
    /// Returns an error if generating the signing message fails, if
    /// signing fails, or if the signature scheme is not recognized.
    pub fn sign_as_sender_with_domain<A: Account>(
        &mut self,
        sender: &A,
        domain: &[u8],
    ) -> AptosResult<()> {
        let signing_message = self.fee_payer_txn.signing_message_with_domain(domain)?;
    ...
    pub fn sign_as_secondary<A: Account>(&mut self, index: usize, signer: &A) -> AptosResult<()> {
        self.sign_as_secondary_with_domain(index, signer, b"APTOS::RawTransactionWithData")
    }

    /// Like [`sign_as_secondary`](Self::sign_as_secondary) but with a
    /// caller-supplied signing domain.
    ///
    /// # Errors
    ///
    /// Returns an error if the index is out of bounds, if generating
    /// the signing message fails, if signing fails, or if the
    /// signature scheme is not recognized.
    pub fn sign_as_secondary_with_domain<A: Account>(
        &mut self,
        index: usize,
        signer: &A,
        domain: &[u8],
    ...
    pub fn sign_as_fee_payer<A: Account>(&mut self, fee_payer: &A) -> AptosResult<()> {
        self.sign_as_fee_payer_with_domain(fee_payer, b"APTOS::RawTransactionWithData")
    }

    /// Like [`sign_as_fee_payer`](Self::sign_as_fee_payer) but with a
    /// caller-supplied signing domain.
    ///
    /// # Errors
    ///
    /// Returns an error if generating the signing message fails, if
    /// signing fails, or if the signature scheme is not recognized.
    pub fn sign_as_fee_payer_with_domain<A: Account>(
        &mut self,
        fee_payer: &A,
        domain: &[u8],
    ) -> AptosResult<()> {
...
pub trait Sponsor: Account + Sized {
    ...
    ///
    /// ```rust,ignore
    /// use move_core_sdk::transaction::Sponsor;
    ///
...
///
/// ```rust,ignore
/// use move_core_sdk::transaction::sponsor_transaction;
///
