...
All notable changes to `move-core-sdk` will be documented in this file.
...
- **Breaking**: Renamed crate from `aptos-sdk` to `move-core-sdk`.
  All internal `aptos_sdk` imports are now `move_core_sdk`. The
  `aptos-sdk` crate is no longer published; use `move-core-sdk`
  or the higher-level `move-rust-sdk` facade instead.

### Added

- `AptosConfig::set_chain_id(chain_id)` -- override the chain ID. The
  default (chain ID derived from the `Network` variant) is unchanged.
  This is the public API the `move-rust-sdk` crate uses to inject a
  non-Aptos chain ID (e.g. Movement's `126` for mainnet, `250` for
  testnet) into an otherwise Aptos-shaped configuration. Pass
  `ChainId::new(0)` to clear the override.
- `AptosConfig::from_parts(fullnode_url, indexer_url, faucet_url)` --
  public structural constructor that validates the URL schemes and
  returns an `AptosConfig` with default timing/retry/pool settings.
  Used by `move-rust-sdk::MoveConfig::into_aptos_config` to bridge a
  chain-agnostic `MoveConfig` into an `AptosConfig` without
  `pub(crate)` field access.
- `RawTransaction::signing_message_with_domain(domain)`,
  `RawTransactionOrderless::signing_message_with_domain(domain)`,
  `MultiAgentRawTransaction::signing_message_with_domain(domain)`,
  `FeePayerRawTransaction::signing_message_with_domain(domain)` --
  like the existing `signing_message()` but with a caller-supplied
  signing domain. The default `signing_message()` is unchanged
  (uses `"APTOS::RawTransaction"` or `"APTOS::RawTransactionWithData"`
  etc.). Use the `_with_domain` variants for non-Aptos Move-based
  chains whose signing domain differs (e.g. Movement's
  `"MOVEMENT::RawTransaction"`).
- `SignedTransaction::hash_with_domain(domain)` and
  `SignedTransactionOrderless::hash_with_domain(domain)` -- like the
  existing `hash()` but with a caller-supplied domain.
- `sign_transaction_with_domain(raw_txn, account, domain)`,
  `sign_multi_agent_transaction_with_domain(...)`,
  `sign_fee_payer_transaction_with_domain(...)`,
  `sign_sponsored_transaction_with_domain(...)`,
  `TransactionBuilder::build_and_sign_with_domain(account, domain)`,
  `PartiallySigned::sign_as_sender_with_domain(sender, domain)`,
  `PartiallySigned::sign_as_secondary_with_domain(index, signer, domain)`,
  `PartiallySigned::sign_as_fee_payer_with_domain(fee_payer, domain)` --
  like the existing `sign_*` and `build_and_sign` helpers but with a
  caller-supplied signing domain. The default `sign_*` and
  `build_and_sign` helpers are unchanged. Use the `_with_domain`
  variants for non-Aptos Move-based chains. Re-exported from
  `aptos_sdk::transaction` alongside the existing helpers.

### Changed

- The `AptosConfig` struct gained a `chain_id_override: Option<ChainId>`
  private field. The default value is `None` and the existing
  constructors (`mainnet`, `testnet`, `devnet`, `local`, `custom`)
  set it to `None`. The override is only used when set via
  `set_chain_id`. There is no observable change for callers who do
  not use `set_chain_id`.
