...
fn generate_entry_function(
        ...
        .map(|(name, _, _)| {
            quote! {
                aptos_bcs::to_bytes(&#name)
                    .map_err(|e| move_core_sdk::error::AptosError::Bcs(e.to_string()))?
            }
        })
        .collect();
    ...
    let type_args_param = if has_type_params {
        quote! { , type_args: Vec<move_core_sdk::types::TypeTag> }
    } else {
        quote! {}
    };
    ...
    quote! {
        #[doc = #full_doc]
        pub fn #fn_name(&self, #(#param_defs),* #type_args_param) -> move_core_sdk::error::AptosResult<move_core_sdk::transaction::TransactionPayload> {
            let args = vec![
                #(#arg_encodings),*
            ];

            let function_id = format!("{}::{}::{}", self.address(), Self::MODULE, #func_name_str);
            let entry_fn = move_core_sdk::transaction::EntryFunction::from_function_id(
                &function_id,
                #type_args_use,
                args,
            )?;

            Ok(move_core_sdk::transaction::TransactionPayload::EntryFunction(entry_fn))
        }
    }
}
...
fn generate_view_function(
    ...
    let arg_encodings_bcs: Vec<_> = params
        ...
        .map(|(name, _, _)| {
            quote! {
                ::move_core_sdk::aptos_bcs::to_bytes(&#name)
                    .map_err(|e| ::move_core_sdk::error::AptosError::Bcs(e.to_string()))?
            }
        })
        .collect();
    ...
    quote! {
        ...
        pub async fn #fn_name(
            &self,
            aptos: &::move_core_sdk::Aptos,
            #(#param_defs),*
            #type_args_param
        ) -> ::move_core_sdk::error::AptosResult<#return_type> {
            let args = vec![
                #(#arg_encodings_bcs),*
            ];
        ...
        pub async fn #fn_name_json(
            ...
            #(#param_defs),*
            #type_args_param
        ) -> ::move_core_sdk::error::AptosResult<Vec<serde_json::Value>> {
            let args = vec![
                #(#arg_encodings_json),*
            ];
...
fn view_arg_encoding(name: &Ident, move_type: &str) -> TokenStream {
    match move_type {
        ...
        t if t.starts_with("vector<u8>") => {
            quote! { serde_json::json!(::move_core_sdk::const_hex::encode(&#name)) }
        }
...
fn move_type_to_rust(move_type: &str) -> TokenStream {
    match move_type {
        ...
        "u64" => quote! { u64 },
        "u128" => quote! { u128 },
        "u256" => quote! { move_core_sdk::types::U256 },
        "address" | "&signer" | "signer" => quote! { move_core_sdk::types::AccountAddress },
        ...
        t if t.contains("::object::Object<") => quote! { move_core_sdk::types::AccountAddress },
